...
public class AwsAtXmlTransferService : BackgroundService
{
    ...
    private async Task<BucketAccelerateStatus> CheckS3Acceleration(AmazonS3Client s3Client, string bucketName)
    {
        ...
        {
            _logger.Error(amazonS3Exception, $"Error occurred retrieving S3 acceleration status for bucket {bucketName}");
        }
        catch (Exception ex)
        {
    ...
    private async Task<int> TransferFilesAsync(Guid lockingGuid, CancellationToken stoppingToken, string bucketName)
    {
        ...
        while (recordsToTransfer.Count > 0 && !stoppingToken.IsCancellationRequested)
        {
            var server = AppSettings.Instance.GetAtXmlS3TransferServers()
                                    .FirstOrDefault(s => s.Key.Equals(Environment.MachineName, StringComparison.OrdinalIgnoreCase));
            ...
            try
            {
                await Parallel.ForEachAsync(recordsToTransfer,
                                            ...
                                            async (record, token) =>
                                            {
                                                ...
                                                try
                                                {
                                                    if (record.TransferStatus == TransferStatus.Retrieved ||
                                                        (record.TransferStatus == TransferStatus.TryAgain &&
                                                         lastAttempt           <= DateTime.UtcNow.AddSeconds(0 - 5 * (2 ^ attempt))))
                                                    {
                                                        count++;
                                                ...
                                                {
                                                    record.TransferStatus = ++attempt >= 10 ? TransferStatus.Error : TransferStatus.TryAgain;
                                                    record.Message =
    ...
    private async Task RecordResultsAsync(AwsAtXmlTransferRecord record, CancellationToken stoppingToken)
    {
        ...
        cmd.Parameters.Add("@Message", SqlDbType.VarChar, -1).Value = string.IsNullOrEmpty(record.Message) ? DBNull.Value : record.Message;
        cmd.Parameters.AddWithValue("@ApplicationTransferId", record.ApplicationTransferId);
    ...
    private async Task ExportToFileAsync(AwsAtXmlTransferRecord record, CancellationToken stoppingToken, string bucketName)
    {
    ...
    private async Task<List<AwsAtXmlTransferRecord>> GetRecordsToTransferAsync(Guid lockingGuid, CancellationToken stoppingToken)
    {
                    ...
                    ret.Add(new AwsAtXmlTransferRecord
                            {
                                ...
                                ArchiveDBName         = reader.IsDBNull(2) ? null : reader.GetString(2),
